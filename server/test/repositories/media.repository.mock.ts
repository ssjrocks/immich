...
export const newMediaRepositoryMock = (): Mocked<RepositoryInterface<MediaRepository>> => {
  return {
    ...
    probe: vitest.fn(),
    extractAudioChunks: vitest.fn().mockResolvedValue([]),
    ...
    extractVideoFrames: vitest.fn().mockResolvedValue({ framePaths: [], effectiveFrameRate: 0.5 }),
    extractVideoFrameAt: vitest.fn().mockResolvedValue(Buffer.from('')),
  };
};
