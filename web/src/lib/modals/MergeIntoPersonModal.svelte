<script lang="ts">
  ...
  onMount(async () => {
    const data = await getAllPeople({ withHidden: true, closestPersonId: person.id });
...
<Modal title={$t('merge_people')} {onClose} size="small">
  <ModalBody>
    <div class="mb-4">
    ...
    {#if candidates.length === 0}
      ...
      {#if searchName.trim()}
        <Button size="small" class="mt-2" disabled={isCreating} onclick={createAndSelect}>
